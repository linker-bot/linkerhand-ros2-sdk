...
class LinkerHandL6RS485:
    ...
    def _read_input_registers(self, address: int, count: int) -> List[int]:
        ...
        result = self.cli.read_input_registers(address, count=count, device_id=self.slave)
        return result.registers if not result.isError() else []

    def _write_register(self, address: int, value: int):
        """写入单个寄存器"""
        time.sleep(_INTERVAL)
        self.cli.write_register(address, value, device_id=self.slave)

    def _write_registers(self, address: int, values: List[int]):
        """写入多个寄存器"""
        time.sleep(_INTERVAL)
        self.cli.write_registers(address, values=values, device_id=self.slave)
